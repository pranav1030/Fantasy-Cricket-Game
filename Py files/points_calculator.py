...
def calculate_points(tuple):
    points = 0.0
    score = tuple[1]
    try:
        strike_rate = float(tuple[1]) / float(tuple[2])  # strike rate =runs/balls faced
    except:
        strike_rate = 0
    fours, sixes = float(tuple[3]), float(tuple[4])

    twos = int((score - 4 * fours - 6 * sixes) / 2)
    wickets = 10 * float(tuple[8])
    try:
        economy = float(tuple[7]) / (float(tuple[5]) / 6)
    except:
        economy = 0
    Fielding = float(tuple[9]) + float(tuple[10]) + float(tuple[11])
...
player_points = {}
for tuple in row:
    player_points[tuple[0]] = calculate_points(tuple)
